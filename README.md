<div align="center">

  <img src="app/src/main/res/mipmap-xxxhdpi/ic_launcher.png" width="150" alt="Koma Logo">

  <h1>Koma (コマ)</h1>

  <p><strong>Full-featured comics reader for Android.</strong></p>
  <p>An open-source reader for manga, manhwa, manhua, comics, and novels. Built on Mihon + TachiyomiSY foundations, tuned for comfort and speed.</p>

  <p>
    <a href="https://github.com/UDIN-K/KOMA/releases/latest">
      <img src="https://img.shields.io/github/v/release/UDIN-K/KOMA?style=for-the-badge&label=%E2%AC%87%20DOWNLOAD%20KOMA&labelColor=1B2838&color=0D1B2A&logo=android&logoColor=3DDC84" alt="Download Koma">
    </a>
    <a href="https://apkpure.com/id/koma/com.koma.reader">
      <img src="https://img.shields.io/badge/OR%20GET%20ON-APKPure-1B2838?style=for-the-badge&logo=android&logoColor=24E98E&color=0D1B2A" alt="Get on APKPure">
    </a>
  </p>

  <p>
    <a href="https://github.com/UDIN-K/KOMA/releases"><img src="https://img.shields.io/github/downloads/UDIN-K/KOMA/total?style=flat-square&color=0D1B2A&labelColor=1B2838&logo=github" alt="Downloads"></a>
    <a href="https://github.com/UDIN-K/KOMA/blob/main/LICENSE"><img src="https://img.shields.io/github/license/udin-k/koma?style=flat-square&color=0D1B2A&labelColor=1B2838" alt="License"></a>
    <a href="https://github.com/UDIN-K/KOMA/releases"><img src="https://img.shields.io/github/v/release/UDIN-K/KOMA?style=flat-square&label=latest&color=0D1B2A&labelColor=1B2838" alt="Latest Release"></a>
  </p>

  <sub>
    <a href="https://github.com/UDIN-K/KOMA/releases">Releases</a> •
    <a href="https://github.com/UDIN-K/KOMA/releases">Preview builds</a> •
    <a href="#translations">Translate</a>
  </sub>

</div>

---


## Koma in one glance

- Built on Mihon + TachiyomiSY, focused on fast navigation and readable pages.
- Signature *Midnight Deep* theme and floating island navigation.
- Global search with filters, exclusions, and saved queries.
- Migration tools and batch actions for organizing large libraries.

---

## Feature map

### Reading flow
- Multiple modes: continuous, paged, webtoon, auto-scroll
- Smart backgrounds that adapt to page content
- Webtoon optimization with forced zoom control
- Offline reading with flexible download caching

### Library control
- Drag-and-drop categories with custom grouping
- Edit covers, notes, and metadata
- Batch operations: clean titles, merge duplicates, mass import
- Quick actions for bookmark, download, and updates

### Discovery & tracking
- Global search with quotes, exclusions, and saved searches
- Sync progress with MyAnimeList, AniList, Kitsu, MangaUpdates, Shikimori, Bangumi
- Manga recommendations via MAL, AniList, and related titles

### Source power-ups
- Built-in enhancements for E-Hentai / ExHentai, Mangadex, 8Muses, NHentai, HBrowse, Puruin, LANraragi
- Seamless source migration with intelligent matching

### Design touches
- Midnight Deep theme plus full theming support
- Floating island navigation with smooth animation

---

## Download

- **Stable** — Latest release: [Download Latest](https://github.com/UDIN-K/KOMA/releases/latest)
- **Preview** — Bleeding-edge builds: [Preview Builds](https://github.com/UDIN-K/KOMA/releases)
- **APKPure** — [Get on APKPure](https://apkpure.com/id/koma/com.koma.reader)

> **Requirements:** Android 6.0 (Marshmallow) or above

---

## Contributing & support

- [Contributing Guidelines](CONTRIBUTING.md)
- [Code of Conduct](CODE_OF_CONDUCT.md)
- [Privacy Policy](PRIVACY.md)
- Bug reports: [GitHub Issues](https://github.com/UDIN-K/KOMA/issues)
- Feature requests: [GitHub Issues](https://github.com/UDIN-K/KOMA/issues)

---

## Credits

<div align="center">

Built on the work of:

[Mihon](https://github.com/mihonapp/mihon) · [TachiyomiSY](https://github.com/jobobby04/TachiyomiSY) · [Tachiyomi](https://github.com/tachiyomiorg/tachiyomi)

<a href="https://github.com/UDIN-K/KOMA/graphs/contributors" title="Contributors">
  <img src="https://contrib.rocks/image?repo=UDIN-K/KOMA" alt="Contributors">
</a>

Thank you to all the people who have contributed!

<a href="https://github.com/mihonapp/mihon/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=mihonapp/mihon" alt="Mihon app contributors" title="Mihon app contributors" width="800" />
</a>

<br>

<sub>Made with Rawrrr XD by <a href="https://github.com/UDIN-K">UDIN-K</a></sub>

</div>
